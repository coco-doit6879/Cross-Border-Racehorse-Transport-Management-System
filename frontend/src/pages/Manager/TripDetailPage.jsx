...
const TripDetailContent = () => {
  ...
  return (
    <section className="operations-page">
      ...
      {!isTripEditable(trip) ? <p className="read-only-note manager-scope-note">Chuyến {TRIP_STATUS_LABELS[trip.status].toLowerCase()} hiển thị phân công ở chế độ chỉ đọc.</p> : null}

      <div className="trip-detail-grid">
        <div className="detail-layout__main">
          <article className="content-card"><div className="content-card__heading"><h2>Thông tin chuyến và đơn</h2><span className={`status-badge status-badge--${trip.status.toLowerCase().replace('_', '-')}`}>{TRIP_STATUS_LABELS[trip.status]}</span></div><dl className="detail-grid"><div><dt>Khách hàng</dt><dd>{trip.customer}</dd></div><div><dt>Biển số xe</dt><dd>{trip.vehiclePlate || 'Chưa có'}</dd></div><div><dt>Bắt đầu dự kiến</dt><dd>{formatDateTime(trip.startAt)}</dd></div><div><dt>Kết thúc dự kiến</dt><dd>{formatDateTime(trip.endAt)}</dd></div><div><dt>Số ngựa</dt><dd>{trip.horseCount}</dd></div><div><dt>Yêu cầu đặc biệt</dt><dd>{trip.specialRequirements || 'Không có'}</dd></div></dl></article>
          <article className="content-card"><h2>Danh sách ngựa</h2><div className="horse-list">{trip.horses.map((horse) => <div className="manager-horse-row" key={horse.id}><span>{horse.name.slice(0, 1)}</span><div><strong>{horse.name}</strong><small>Hộ chiếu: {horse.passport}</small></div></div>)}</div></article>
          <article className="content-card"><h2>Lịch sử phân công / thay đổi</h2>{trip.assignmentHistory.length ? <div className="history-list">{trip.assignmentHistory.map((entry) => <div className="history-item" key={entry.id}><span className="history-item__dot" /><div><strong>{entry.role === 'DRIVER' ? 'Tài xế' : 'Phụ xe'}: {people.get(entry.oldPersonId)?.fullName || 'Chưa phân công'} → {people.get(entry.newPersonId)?.fullName || 'Chưa phân công'}</strong><p>{entry.reason}</p><small>{formatDateTime(entry.changedAt)}</small></div></div>)}</div> : <p className="muted-text">Chưa có thay đổi phân công được ghi nhận.</p>}</article>
        </div>
