...
export default function FleetTripsPage() {
  ...
  return <Space direction="vertical" size={20} style={{ width: '100%' }}><Card title="Phân công chuyến vận chuyển"><Alert type="info" showIcon message="Chuyến chỉ được tạo sau khi chọn xe, tài xế và phụ xe. Biển số không xuất hiện trước thời điểm này." /></Card><Table loading={loading} rowKey="_id" dataSource={rows} columns={[
    ...
    { title: '', render: (_, row) => <Button type={row.route ? 'default' : 'primary'} disabled={row.route && row.route.status !== 'SCHEDULED'} onClick={() => open(row)}>{row.route ? 'Đổi phân công' : 'Phân công'}</Button> }
  ]} />
